# hybrid.py - Hybrid candidate retrieval with stage-level instrumentation
import sys
...
def keyword_search(query: str, k: int = 20, filters: dict | None = None) -> list[dict]:
    if config.FORCE_KEYWORD_FAILURE:
        raise RuntimeError("Simulated keyword retrieval outage")
    ...
    return [
        {
            "chunk_id": row[0], "content": row[1], "source_file": row[2],
            "doc_type": row[3], "category": row[4], "chunk_index": row[5],
            "page_number": row[6], "keyword_rank": rank + 1,
    ...
    return [
        {
            "chunk_id": row[0], "content": row[1], "source_file": row[2],
            "doc_type": row[3], "category": row[4], "chunk_index": row[5],
            "page_number": row[6], "keyword_rank": None, "keyword_score": None,
            "semantic_rank": rank + 1, "semantic_score": float(row[7]),
            "matched_by": "semantic",
        }
        for rank, row in enumerate(rows)
    ]
...
def hybrid_candidates(query: str, k: int = 20, filters: dict | None = None) -> list[dict]:
    """Hybrid retrieval with graceful degradation. Logs per-side counts."""
    kw: list[dict] = []
    kw_error = None
    try:
        with span("retrieval.keyword"):
